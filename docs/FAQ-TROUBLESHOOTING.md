...
| **Tails: VeraCrypt não existe** | VeraCrypt não é empacotado no Tails | Usar LUKS Persistent Storage (já cifrado). Ver [Guia Tails COMANDO T.1](../tails/🐧%20Zero-Trust-Core-Tails.md#-comando-t1-configurar-persistent-storage--additional-software) |
| **Tails: KeePassXC não persiste após reboot** | Não marcado como Additional Software | Instalar com `sudo apt install keepassxc` e aceitar "Install Every Time". Ver [Playbook T01](../tails/playbooks/T01-tails-cofre-luks.md) |
...
| **`VBoxManage: command not found`** | VirtualBox não instalado no host | Siga [W00 — Instalar VirtualBox](../whonix/playbooks/W00-instalar-configurar-virtualbox.md) ou `ztc-whonix-install-virtualbox.sh` |
| **`vboxdrv: No such device` / VM não inicia** | Secure Boot ou DKMS sem headers | W00 passo 7 — desabilitar Secure Boot ou assinar módulos MOK; confirmar `linux-headers-$(uname -r)` |
| **Whonix `.ova` com Xfce no nome** | Variante descontinuada | Baixar **LXQt** ou **CLI** em [whonix.org/wiki/VirtualBox](https://www.whonix.org/wiki/VirtualBox) |
| **Gateway sobe, Workstation sem Tor** | Ordem de boot ou Anon Connection Wizard incompleto | Inicie **Gateway primeiro**; complete o wizard; rode `systemcheck` na Workstation |
| **USB pendrive não aparece na VM Whonix** | Extension Pack ou grupo `vboxusers` | W00 passo 8 (`-e` no script); logout/login após `usermod -aG vboxusers` |
